...
public class ArtikelDAO implements workshop.dao.ArtikelDAOInterface {
	...
	@Override
	public long createArtikel(Artikel artikel) {
	...
	@Override
	public Artikel getArtikelWithArtikelId(long artikel_id) {
		return this.em.find(Artikel.class, artikel_id);
	}

	@Override
	public Artikel getArtikelWithNaam(String naam) {
		return (Artikel) this.em.createQuery("SELECT * FROM " + Artikel.class.getSimpleName() + 
				"WHERE artikel_naam = " + naam).getResultList().get(0);
	}

	@Override
	public List<Artikel> getArtikellijst() {
		return this.em.createNativeQuery("SELECT * FROM ", Artikel.class).getResultList();
	}

	@Override
	public void updateArtikel(long id, Artikel artikel) {
		Artikel oud = getArtikelWithArtikelId(id);
		this.em.getTransaction().begin();
		oud.setNaam(artikel.getNaam());
		oud.setOmschrijving(artikel.getOmschrijving());
		oud.setPrijs(artikel.getPrijs());
		this.em.getTransaction().commit();
	}

	@Override
	public void deleteArtikelWithArtikelId(long artikel_id) {
		this.em.remove(getArtikelWithArtikelId(artikel_id));
	}
